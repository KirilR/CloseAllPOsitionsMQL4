void CloseOrdersIf()
{
   //adding KAMA Variables
   double CurrentSlow=iCustom(NULL,0,"KAMA",KamaSlow1,KamaSlowSwitch,KamaSlow2,0,1);
   double CurrentFast = iCustom(NULL,0,"KAMA",KamaFast1,KamaFastSwitch,KamaFast2,0,1);
   ...
   for (int i = (OrdersTotal() - 1); i >= 0; i--)
   {
      ...
      if ((OrderType() == OP_BUY)&&(CurrentFast<CurrentSlow))
          {
         res = OrderClose(OrderTicket(), OrderLots(), BidPrice, Slippage);
         Comment("CloseBUY");
      }
      else if ((OrderType() == OP_SELL)&&(CurrentFast>CurrentSlow))
          {
